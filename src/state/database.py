...
class Database:
    ...
    async def upsert_device(
        ...
        await self._db.execute(
            """INSERT INTO devices (serial_number, ip, hostname, friendly_name, registration, status)
               VALUES (?, ?, ?, ?, ?, ?)
    ...
    async def update_status(self, serial_number: str, status: dict) -> None:
        await self._db.execute(
    ...
    async def upsert_desired_state(self, serial_number: str, register_set_values: dict, quality_preset: str | None = None) -> None:
        import time

        await self._db.execute(
            """INSERT INTO desired_state (serial_number, register_set_values, quality_preset, updated_at)
               VALUES (?, ?, ?, ?)
               ON CONFLICT(serial_number) DO UPDATE SET
            ...
            (serial_number, json.dumps(register_set_values), quality_preset, time.time()),
        )
        await self._db.commit()
