...
DEVICE_PREFIXES = ["VMC", "VML", "ABC", "FB"]
...
class Camera(Device):
    ...
    def send_initial_register_set(
        ...
        if self.model_number.startswith("VMC5040"):
            register_set = Message(copy.deepcopy(arlo.messages.REGISTER_SET_INITIAL_ULTRA))
        elif self.model_number.startswith("FB1001"):
            register_set = Message(copy.deepcopy(arlo.messages.REGISTER_SET_INITIAL_FLOODLIGHT))
        else:
            register_set = Message(copy.deepcopy(arlo.messages.REGISTER_SET_INITIAL_SUBSCRIPTION))
    ...
    def pir_led(self, args):
        ...
        register_set["SetValues"] = {"PIREnableLED": enabled, "PIRLEDSensitivity": sensitivity}
    ...
    def set_quality(self, args):
        ...
        if quality == "low":
            ra_params = Message(
                copy.deepcopy(
                    arlo.messages.RA_PARAMS_FLOODLIGHT
                    if self.model_number.startswith("FB1001")
                    else arlo.messages.RA_PARAMS_LOW_QUALITY
                )
            )
            register_set = Message(
                copy.deepcopy(
                    arlo.messages.REGISTER_SET_LOW_QUALITY_FLOODLIGHT
                    if self.model_number.startswith("FB1001")
            ...
            register_set = Message(
                copy.deepcopy(
                    arlo.messages.REGISTER_SET_MEDIUM_QUALITY_FLOODLIGHT
                    if self.model_number.startswith("FB1001")
            ...
            register_set = Message(
                copy.deepcopy(
                    arlo.messages.REGISTER_SET_HIGH_QUALITY_FLOODLIGHT
                    if self.model_number.startswith("FB1001")
